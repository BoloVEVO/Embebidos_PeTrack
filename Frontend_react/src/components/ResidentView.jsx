...
const SECTIONS = [
  ...
  { key: "live", label: "Video de detecciones", hint: "En vivo y grabaciones" },
...
function DetectionSummary({ detection, active, onClick, selectable, checked, onToggle }) {
  ...
  return <button type="button" className={`resident-detection-row${active ? " active" : ""}`} onClick={onClick}>
    ...
    <span className="badge">{detection.device_id}</span>
  </button>;
}
    ...
    <main className="staff-content">
      {section === "recent" && <section>
        ...
        <div className="resident-detection-layout">
          <div className="resident-detection-list">{detections.map((item) => <DetectionSummary key={item.id} detection={item} active={selectedId === item.id} onClick={() => setSelectedId(item.id)} />)}{!error && !detections.length && <div className="empty-state"><strong>Sin detecciones</strong>No hay actividad reciente para tus dispositivos.</div>}</div>
          <article className="resident-detection-detail">{selected ? <><img src={photoUrl(selected.photo_id)} alt="Captura de la detección" /><div className="detail-heading"><div><span className="auth-step">CAPTURA VERIFICADA</span><h2>{selected.pets?.length || 1} {(selected.pets?.length || 1) === 1 ? "mascota cercana registrada" : "mascotas cercanas registradas"}</h2></div></div><div className="detail-properties"><div><span>Hora de captura</span><strong>{selected.ts ? new Date(selected.ts).toLocaleString() : "No disponible"}</strong></div><div><span>ID detección</span><strong>{selected.id}</strong></div><div><span>Dispositivo main</span><strong>{selected.device_id}</strong></div><div><span>Mascotas detectadas</span><strong>{(selected.pets?.length ? selected.pets : [{ pet: selected.pet, pet_id: selected.pet_id }]).map((item) => `${item.pet || "Mascota cercana registrada"} (${item.pet_id})`).join(", ")}</strong></div></div></> : <div className="empty-state"><strong>Selecciona una detección</strong>La imagen y hora de captura aparecerán aquí.</div>}</article>
        </div>
      </section>}
      ...
      {section === "devices" && <section>
        ...
        <div className="resident-device-groups">
          ...
          <div><div className="resident-device-group-title"><span>Collares asociados</span><b>{devices.collars.length}</b></div><div className="resident-collar-grid">{devices.collars.map((collar) => <article className="resident-collar-card" key={collar.device_id}><div><span className={`device-state ${collar.online ? "online" : "collar"}`} /><span><span className="auth-step">ESP32-C3 · COLLAR</span><h3>{collar.device_id}</h3></span></div><dl><div><dt>Mascota</dt><dd>{collar.pet_id || "Sin asignar"}</dd></div><div><dt>Main asignado</dt><dd>{collar.main_device_id}</dd></div><div><dt>Detectado por</dt><dd>{collar.detected_by_main_id || "Ningún main"}</dd></div><div><dt>Última detección</dt><dd>{collar.last_seen ? new Date(collar.last_seen).toLocaleString() : "Nunca"}</dd></div><div><dt>Estado</dt><dd>{collar.online ? "Detectado recientemente" : "Fuera de alcance"}</dd></div></dl></article>)}</div></div>
        </div>
      </section>}

      {section === "live" && <section>
        <div className="section-header"><div><h1 className="page-title">Video de detecciones</h1><p className="page-sub">Transmisiones activas y videos codificados por el servidor.</p></div><button className="btn ghost sm" onClick={refresh}>Actualizar</button></div>
        <div className="video-session-grid">{videoSessions.map((session) => <article className="video-session-card" key={session.id}><div className="row" style={{justifyContent:"space-between"}}><span className={`badge ${session.status === "live" ? "resolved" : ""}`}>{session.status === "live" ? "● Ocurriendo ahora" : session.status === "processing" ? "Codificando video" : session.status === "completed" ? "Finalizada" : "Error de video"}</span><small>{new Date(session.started_at).toLocaleString()}</small></div>{session.status === "live" ? <img src={liveVideoUrl(session.id)} alt="Detección en vivo" /> : session.video_ready ? <video controls preload="metadata" src={recordedVideoUrl(session.id)} /> : <div className="video-processing">{session.status === "failed" ? session.error : "Preparando MP4…"}</div>}<div className="meta">{session.frame_count} imágenes · {session.device_id}</div></article>)}{!videoSessions.length && <div className="empty-state"><strong>Sin sesiones de video</strong>Cuando un collar sea detectado aparecerá aquí en vivo.</div>}</div>
      </section>}
      ...
      {section === "report" && <section>
        ...
        <div className="resident-report-layout"><div className="resident-detection-list report-select-list">{detections.map((item) => <DetectionSummary key={item.id} detection={item} selectable checked={reportIds.has(item.id)} onToggle={() => toggle(item.id)} onClick={() => toggle(item.id)} />)}{!detections.length && <div className="empty-state"><strong>Sin detecciones disponibles</strong>Necesitas una detección válida para crear un reporte.</div>}</div>
